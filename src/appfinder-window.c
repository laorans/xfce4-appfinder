...
typedef struct _XfceEvalEngine
{
  exprFuncList *m_f;
  exprValList  *m_v;
  exprValList  *m_c;
...
struct _XfceAppfinderWindow
{
  ...
  XfceEvalEngine	     *eval_engine;

  GtkWidget                  *paned;
  GtkWidget                  *entry;
  GtkWidget                  *entry_eval;
  GtkWidget                  *image;
  GtkWidget                  *view;
...
static void
xfce_delete_eval_engine(XfceEvalEngine *engine)
{
  if (engine->m_f) exprFuncListFree(engine->m_f);
  if (engine->m_v) exprValListFree(engine->m_v);
  if (engine->m_c) exprValListFree(engine->m_c);
  free(engine);
}

static XfceEvalEngine *
xfce_new_eval_engine(void)
{
  XfceEvalEngine *engine;
  engine = (XfceEvalEngine *) malloc(sizeof(XfceEvalEngine));
  memset(engine, 0, sizeof(XfceEvalEngine));

  /* Create function list */
  int err = exprFuncListCreate(&engine->m_f);
  if(err != EXPR_ERROR_NOERROR) { printf("Function List Creation Error %d\n", err); goto error; }

  /* Initialize internal functions */
  err = exprFuncListInit(engine->m_f);
  ...
  err = exprValListCreate(&engine->m_v);
  if(err != EXPR_ERROR_NOERROR) { printf("Variable List Creation Error %d\n", err); goto error; }

  /* Create the constant list */
  err = exprValListCreate(&engine->m_c);
  if(err != EXPR_ERROR_NOERROR) { printf("Constant List Creation Error %d\n", err); goto error; }

  /* Initialize internal constants */
  err = exprValListInit(engine->m_c);
  if(err != EXPR_ERROR_NOERROR) {
 ...
 error:
  xfce_delete_eval_engine(engine);
  return NULL;
}
...
xfce_appfinder_window_init (XfceAppfinderWindow *window)
{
  ...
  align = gtk_alignment_new (0.5, 0.0, 0.6, 0.0);
  gtk_box_pack_start (GTK_BOX (vbox2), align, TRUE, TRUE, 0);
  gtk_widget_show (align);

  window->eval_engine = xfce_new_eval_engine();

  window->entry_eval = entry_eval = gtk_entry_new ();
  gtk_container_add (GTK_CONTAINER (align), entry_eval);
  ...
  if (gtk_widget_get_visible (window->paned))
  ...
  else
    {
      ...
      if (text[0] == '=') {
        ...
        iconText = "gnome-calculator";
        if (text[1]) {
          XfceEvalEngine *engine = window->eval_engine;
          if (engine) {
            exprObj *o;
            int err = exprCreate(&o, engine->m_f, engine->m_v, engine->m_c, NULL, 0);
            if (!err) {
              char expr[2000];
              snprintf(expr, 2000, "ans=%s;", &text[indexExpr]);
              ...
              if (!err) {
                ...
                if(!err) {
                  char buffer[30];
                  if (text[1] == 'o' || text[1] == 'x') {
                    snprintf(buffer, 30, format, (long long) result);
                  }
                  else snprintf(buffer, 30, format, result);
                  gtk_entry_set_text((GtkEntry *) window->entry_eval, buffer);
                }
              }
            }
            exprFree(o);
            gtk_widget_show (window->entry_eval);
          }
        }
      }
      else gtk_widget_hide (window->entry_eval);
      pixbuf = xfce_appfinder_model_get_icon_for_command (window->model, iconText);
